...
#define AEDPRINTF(...) if(!myrank) printf(__VA_ARGS__)
  ...
  public:
    ...
    int myrank,size;
#ifdef HAVE_MPI
    // Communicator for each case (may be smaller than MPI_COMM_WORLD)
    MPI_Comm comm;
#endif

    // comentado porque en otro lugar hay una generica, para arbol de T
#if 0
    void lisp2tree2(string s, tree<int> &T, unsigned int &i,
    ...
    void lisp2tree2(string s, tree<int> &T) {
      unsigned int z=0;
      lisp2tree2(s,T,z,T.begin());
    }
#endif

    eval_base_t() : dumptests(0), jsonvrbs(-1), trustuser(0),
                    facmax(NAN), fac(NAN)  {
#ifdef HAVE_MPI
      comm = MPI_COMM_WORLD;
  ...
  public:
    ...
    void make_random_btree(btree<int> &t,btree<int>::iterator n,
      ...
      nivel=double(level);
      lambda = level ? 1.0/(siblings/nivel+1.0) : 0.0;
      for (int j=0;j<2;j++) {
        if  (j==0) {
          c=n.left();}
        else {
    ...
    // -----------------------------------------------------------------
    // node values are in [0,m)
    // siblings is the average number of siblings
...
#ifdef HAVE_MPI
    void mpi_json_gather(json &outuser,MPI_Comm comm) {
      // The json `outuser' from each processor is joined
      // in a big json array where outuserall[p] contains the
      // outuser from rank `p'
      int myrank,size;
      MPI_Comm_rank(comm,&myrank);
      MPI_Comm_size(comm,&size);
      json outuserall;
      if (!myrank) {
        // First put my outuser in position 0
        outuserall[0] = outuser;
        for (int p=1; p<size; p++) {
          // Receive the serialized version of the outuser from each process
          // Receive the Nbr of chars
          int nchar;
          MPI_Recv(&nchar,1,MPI_INT,p,0,comm,MPI_STATUS_IGNORE);
          // Dimension the char buffer
          vector<char> w(nchar);
          // Receive the chars
          MPI_Recv(w.data(),nchar,MPI_CHAR,p,0,comm,MPI_STATUS_IGNORE);
          // Convert the chars to a json and store in position `p'
          // of the global json
          outuserall[p] = json::parse(w.data());
        }
        // Convert the global json to a char arraya and Bcast
        string wall = outuserall.dump();
        int nchar = wall.size()+1;
        // Send the Nbr of chars
        MPI_Bcast(&nchar,1,MPI_INT,0,comm);
        // Convert to vector<char>
        vector<char> z(nchar);
        memcpy(z.data(),wall.c_str(),nchar);
        // Send the chars
        MPI_Bcast(z.data(),nchar,MPI_CHAR,0,comm);
      } else {
        // Convert my json to string
        string z = outuser.dump();
        // Nbr of chars in my string
        int nchar = z.size()+1;
        // Send my amount of chars
        MPI_Send(&nchar,1,MPI_INT,0,0,comm);
        // Send my chars
        MPI_Send((char *)z.c_str(),nchar,MPI_CHAR,0,0,comm);
        // Now the master has the global json. Receive that
        // Get the nbr of chars
        MPI_Bcast(&nchar,1,MPI_INT,0,comm);
        // Dimension the local string
        vector<char> wall(nchar);
        // Get the chars
        MPI_Bcast(wall.data(),nchar,MPI_CHAR,0,comm);
        outuserall = json::parse(wall.data());
      }
      // Now all the processes have the outuserall.
      // We plainly replace the outuser by the joined outuserall
      outuser = outuserall;
    }
#endif
    ...
    void run_case_parallel(json &dataall,json &outuser,int &szcase) {
#ifdef HAVE_MPI
      szcase = dataall.size();
      assert(szcase<=size);
      // Create a communicator for this case
      int color = (myrank<szcase? 0 : MPI_UNDEFINED);
      MPI_Comm_split(MPI_COMM_WORLD,color,myrank,&comm);
      json datain;
      if (myrank<szcase) datain = dataall[myrank];
#else
      json datain = dataall;
      szcase = 1;
#endif
...
#ifdef HAVE_MPI
        // If in parallel environment gather output from
        // all processsors in one array
        mpi_json_gather(outuser,comm);
        MPI_Comm_free(&comm);
#endif
      }
    }

    int ejerc,dumptests,jsonvrbs,trustuser;
    string testfile;
    double facmax, fac;
    void evfun(int vrbs=0,int ucase=-1) {
      ...
      int caso = 0, okt = 0;
      for (auto p:tests) {
        if (ucase<0 || ucase==caso) {
          json outuser,
            &dataall = p["datain"],
            &outref = p["output"];
          int szcase;
          // cout << dataall << endl;
          run_case_parallel(dataall,outuser,szcase);
          if (!myrank) {
            // check_case runs only in the master
            int ok = check_case(dataall,outref,outuser);
            if (!ok || vrbs) {
              AEDPRINTF("\nEJ%d|CASO %0d\n",ejerc,caso);
#ifdef HAVE_MPI
              for (int j=0; j<szcase; j++) {
                if (size>1) cout << "===== In rank " << j << " =====" << endl;
                cout << "--------- Input data: " << endl;
                cout <<  dataall[j].dump() << endl;
                cout << "--------- Output(user): " << endl;
                cout <<  outuser[j].dump() << endl;
                cout << "--------- Output(ref): " << endl;
                cout <<  outref[j].dump() << endl;
              }
              if (size>1) cout << "====================" << endl;
#else
              cout << "--------- Input data: " << endl;
              cout <<  dataall.dump() << endl;
              cout << "--------- Output(user): " << endl;
            ...
            if (dumptests && !myrank) {
              json thiscase;
              thiscase["datain"] = dataall;
              thiscase["output"] = (trustuser? outuser : outref);
              thiscase["ucase"] = caso;
              testso.push_back(thiscase);
            }
            okt += ok;
          }
        }
      ...
      if (dumptests && !myrank) {
        ofstream out("./dumptests.json");
        out << testso.dump(jsonvrbs) << endl;
      }

      if (ucase<0 && !myrank) {
        int total = caso;
        AEDPRINTF("ESTADO EJ%d Total %d, ok %d, mal %d. Todos bien? %s\n",
    ...
    int evrandom(int seed,int vrbs=0,int hardness=0) {
      ...
      for (int caso=0; caso<ncasos; caso++) {
        if (vrbs>=2 && !hardness)
          AEDPRINTF("Ejerc %d. Caso %d\n",ejerc,caso);

        json datain,outuser;
        uint32_t seed = rnd.getseed();
#ifdef HAVE_MPI
        // Be sure that all procs use the same seed
        ...
        chrono.start();
        int szcase;
        run_case_parallel(datain,outuser,szcase);
        chrono.stop();
        if (dumptests && !myrank) {
          json thiscase;
        ...
        // cout << outuser.dump() << " " << hasher.val()%1000 << endl;
        hasher.hash(outuser.dump());
      }

      if (dumptests && !myrank) {
        ofstream out("./dumptests.json");
        out << tests.dump(jsonvrbs) << endl;
      }
      ...
      if (vrbs && !hardness)
        AEDPRINTF("EJ%d: SEED=%03d, HASH=%03d\n",ejerc,seed,H);
      return H;
    }
    ...
    template<typename T>
    void random_shuffle(randomg_t &rnd,vector<T> &v,int k1,int k2) {
      int n = k2-k1;
      if (n<=1) return;
    ...
    template<typename T>
    void random_shuffle(randomg_t &rnd,vector<T> &v) {
